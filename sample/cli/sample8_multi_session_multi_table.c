...
SQLBIGINT appendClose(SQLHENV aEnv, SQLHDBC aCon, SQLHSTMT aStmt);
...
int appendF1(SQLHENV aEnv, SQLHDBC aCon, SQLHSTMT aStmt, FILE *aFp)
{
    ...
    if( fscanf(aFp, "%s %s %hd %s %s %hd %lld %lld\n",
               sData[0], sData[1], &sParam[2].mShort,
               sData[2], sData[3], &sParam[5].mShort,
               &sParam[6].mLong, &sParam[7].mLong) != 8 )
    {
...
int appendEvent(SQLHENV aEnv, SQLHDBC aCon, SQLHSTMT aStmt, FILE* aFp)
{
    ...
    if( fscanf(aFp, "%s %d %s\n",
               sData[0], &sParam[1].mInteger, sData[1]) != 3 )
    {
        printf("file read error\n");
        return RC_FAILURE;
    }
